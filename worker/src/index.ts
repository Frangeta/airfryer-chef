...
import { callForcedTool, callChat } from './anthropic';
...
function jsonSchemaFor(schema: z.ZodTypeAny) {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // --- Parseo y validación del contexto estructurado ----------------------
    let body: any;
    try {
      ...
      switch (url.pathname) {
        case '/generate': {
          ...
          const input = await callForcedTool({
            ...
            tool: {
              ...
              description: 'Devuelve entre 2 y 4 propuestas de receta estructuradas para Air Fryer.',
              input_schema: jsonSchemaFor(AIRecipeProposalsSchema)
            },
            maxTokens: 4000
          });
          return json(AIRecipeProposalsSchema.parse(input), 200, origin);
        }
        ...
        case '/substitute': {
          ...
          const input = await callForcedTool({
            ...
            tool: {
              ...
              description: 'Devuelve entre 1 y 3 sustituciones de ingrediente clasificadas por impacto en el resultado.',
              input_schema: jsonSchemaFor(SubstitutionsSchema)
            },
            maxTokens: 1200
          });
          return json(SubstitutionsSchema.parse(input), 200, origin);
        }
        ...
        case '/convert': {
          ...
          const input = await callForcedTool({
            ...
            tool: {
              name: 'convert_recipe',
              description: 'Convierte una receta tradicional a parámetros de Air Fryer.',
              input_schema: jsonSchemaFor(AIConvertedRecipeSchema)
            },
            maxTokens: 1200
          });
          return json({ converted: AIConvertedRecipeSchema.parse(input) }, 200, origin);
        }

        case '/chat': {
          const context = AIContextSchema.parse(body.context);
          const messages = Array.isArray(body.messages) ? body.messages : [];
          const { reply, toolInput } = await callChat({
            apiKey: env.ANTHROPIC_API_KEY,
            model,
            system: `${buildSystemPrompt()}\n\nEstás en un chat conversacional: responde de forma breve y natural en texto, y solo usa la herramienta "propose_recipes" cuando el usuario tenga ya claro qué quiere cocinar.`,
            messages: [
            ...
            tool: {
              name: 'propose_recipes',
              description: 'Úsala SOLO si ya tienes suficiente información para proponer una receta concreta.',
              input_schema: jsonSchemaFor(AIRecipeProposalsSchema)
            }
          });
          return json(
            { reply, suggestedRecipe: toolInput ? AIRecipeProposalsSchema.parse(toolInput) : undefined },
            200,
            origin
          );
        }
